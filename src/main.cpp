...
bool isHostLittleEndian() {
  const std::uint16_t marker = 0x0001U;
...
template <typename T>
bool writeBinaryLittleEndian(std::ostream& output, const T& value) {
  static_assert(std::is_trivially_copyable<T>::value, "Binary PLY writer requires trivially copyable values.");
  ...
  output.write(reinterpret_cast<const char*>(bytes), static_cast<std::streamsize>(sizeof(T)));
...
bool writeMeshAsPly(const fs::path& path, const std::vector<Triangle>& triangles) {
  ...
  for (const Triangle& triangle : filtered) {
    ...
    for (const Vec3f& vertex : vertices) {
      if (!writeBinaryLittleEndian(output, vertex.x) ||
          !writeBinaryLittleEndian(output, vertex.y) ||
          !writeBinaryLittleEndian(output, vertex.z) ||
          !writeBinaryLittleEndian(output, unit_normal.x) ||
          !writeBinaryLittleEndian(output, unit_normal.y) ||
          !writeBinaryLittleEndian(output, unit_normal.z)) {
        return false;
      }
    }
  }
  ...
  for (std::size_t face_index = 0; face_index < filtered.size(); ++face_index) {
    const unsigned char vertex_per_face = 3U;
    const std::int32_t base = static_cast<std::int32_t>(face_index * 3U);
    const std::int32_t indices[3] = {base, base + 1, base + 2};
    if (!writeBinaryLittleEndian(output, vertex_per_face) ||
        !writeBinaryLittleEndian(output, indices[0]) ||
        !writeBinaryLittleEndian(output, indices[1]) ||
        !writeBinaryLittleEndian(output, indices[2])) {
      return false;
    }
  }
